...
import {
  Home, Layers, ShieldCheck, Sliders, TrendingUp, Activity, Compass,
...
`;
  ...
  const checkHealth = async () => {
    ...
    if (health.isHealthy) {
      ...
      setApiError(health.error || 'API Server Disconnected.');
      return false;
    }
  };
  ...
  // Auto-reconnect polling: when OFFLINE, check backend health every 5 seconds
  useEffect(() => {
    if (apiStatus !== 'OFFLINE') return;
    const timer = setInterval(async () => {
      const isOnline = await checkHealth();
      if (isOnline) {
        showToast('Connected to Backend API Server!');
  ...
  const apiFetch = async (path: string, options: RequestInit) => {
    return utilityApiFetch(path, options);
  };

  const runValuationInference = async (modelsOverride?: string[]) => {
    const modelsToUse = modelsOverride || selectedModels;
    ...
    try {
      ...
      const explainRes = await apiFetch('/explain', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      });
      ...
      const cfRes = await apiFetch('/counterfactual', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      });
  ...
  const handleCompareAllModels = () => {
    const all7 = ['xgboost', 'lightgbm', 'catboost', 'random_forest', 'gradient_boosting', 'linear_regression', 'mlp'];
    setSelectedModels(all7);
    runValuationInference(all7);
  };
  ...
  return (
    <div className="app-layout" data-theme={theme}>
      ...
      <main className="main-wrapper">
        ...
        {/* Global Error Banner */}
        {apiError && (
          <div style={{ background: 'var(--bg-red-light)', border: '1px solid var(--danger-red)', color: 'var(--danger-red)', padding: '12px 20px', borderRadius: '10px', marginBottom: '24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', fontSize: '14px' }}>
            ...
            <button
              onClick={async () => {
                const isOnline = await checkHealth();
                if (isOnline) {
                  showToast('Reconnected to Backend API!');
                  runValuationInference();
                } else {
                  showToast('Backend still unreachable. Ensure uvicorn server is running.');
                }
              }}
              style={{ padding: '6px 14px', borderRadius: '6px', background: 'var(--danger-red)', color: '#fff', border: 'none', cursor: 'pointer', fontWeight: 600, fontSize: '12px', display: 'flex', alignItems: 'center', gap: '6px', whiteSpace: 'nowrap' }}
            >
            ...
            <div className="dashboard-sections-grid">
              ...
              <div className="saas-card">
                ...
                <div style={{ marginTop: '20px', textAlign: 'center' }}>
                  <a href="#market" onClick={() => setActiveTab('Market')} style={{ color: 'var(--primary-blue)', fontSize: '13px', fontWeight: 600, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
                    View Detailed Market Insights <ChevronRight size={14} />
                  </a>
                </div>
              </div>
            </div>
        ...
        {activeTab === 'Analysis' && (
          <PropertyAnalysisView
            ...
            shapPositives={shapPositives}
            shapNegatives={shapNegatives}
            onEditDetails={() => setIsModalOpen(true)}
          />
        )}

        {activeTab === 'Market' && (
          <MarketInsightsView city={city} prediction={prediction} />
        )}
          ...
          <div className="modal-card" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ marginTop: '16px' }}>
              <ModelSelectionPanel
                ...
                onCompareAllModels={handleCompareAllModels}
              />
            </div>

            <div style={{ display: 'flex', gap: '12px', marginTop: '16px' }}>
              <button className="btn-primary-lg" onClick={() => runValuationInference()} disabled={loading || selectedModels.length === 0} style={{ flex: 1 }}>
